...
class ProxyFetcher(object):
    ...
    def freeProxy02():
        ...
        githubfree代理池
        """
        logout("proxyFetcher", f"02程序启动")

        # 操作redis数据库对象
        proxy_handler = ProxyHandler()

        # 实时爬取代理
        from fetcher.getGithub import getRaw
        githubRaw = getRaw()

        # log可读性相关参数
        proxyNums = len(githubRaw)
        proxyList = githubRaw
        proxyCount = 0
        proxyCount_successful = 0
    ...
    def freeProxy03():
        ...
        githubfree代理池
        """
        logout("proxyFetcher", f"03程序启动")

        # 操作redis数据库对象
        proxy_handler = ProxyHandler()

        # 实时爬取代理
        from fetcher.getVpnse import  getRaw
        githubRaw = getRaw()
